...
export function buildSoapGenerationInput(input: GenerateSoapNoteInput): string {
  ...
  return [
    ...
    "- Place each relevant ICD-10 code INSIDE the Assessment section, on its own",
    "  line beneath the problem it supports, formatted as: CODE - Description.",
    "- Do NOT output a separate ICD-10 section after Plan or anywhere outside",
    "  Assessment.",
    '- If none fit, write "No supported ICD-10 code available from local catalog."',
    "  within the Assessment section.",
    "",
    "Patient history context rules:",
    "- PATIENT HISTORY CONTEXT is background carried over from this patient's prior",
    "  encounters. The CURRENT TRANSCRIPT is authoritative for today's visit.",
    "- Use prior history only where it is clinically relevant to the current",
    "  presentation, assessment, or plan: chronic conditions, active medications,",
    "  allergies, or comorbidities that affect treatment choices or safety.",
    "- When a known comorbidity bears on today's plan (e.g. a prescription that",
    "  warrants caution given a prior condition), note it briefly.",
    "- Do NOT pull unrelated prior problems into this note simply because they",
    "  exist; a focused visit should stay focused on the presenting complaint.",
    "- Do not contradict the history, but prefer current findings when they differ,",
    "  and never present prior context as if it were observed at today's visit.",
    "- If no prior history is provided, document from the transcript alone.",
    "",
